...
<section class="flex flex-col items-center">
	...
	<button
		class="bg-bridge-sidebar hover:bg-bridge-bg text-white font-bold rounded-3xl mt-5 py-2 w-[70%]"
		on:click={login}>Continue</button
	>

	{#if credentialError}
		<p in:fade class="text-bridge-sidebar text-sm mt-2">*{errorMessage}</p>
	{/if}
...
<style>
	...
	.effect-20 ~ .focus-border:after {
		...
		width: 0;
		height: 2px;
		background-color: var(--bridge-sidebar);
		transition: 0.3s;
	}
	.effect-20 ~ .focus-border:after {
		top: auto;
